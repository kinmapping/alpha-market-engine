...
case "$ACTION" in
  ...
  clear)
    ...
    echo "Consumer group destroyed. Restart strategy-module to recreate it."
    echo ""
    echo "To restart strategy-module, run:"
    echo "  docker-compose -f docker-compose.local.yml restart strategy-module"
    ;;
  clear-all)
    ...
    echo "All consumer groups destroyed. Restart strategy-module to recreate them."
    echo ""
    echo "To restart strategy-module, run:"
    echo "  docker-compose -f docker-compose.local.yml restart strategy-module"
    ;;
  ack-all)
    ...
    # strategy-module コンテナ内で実行（redis パッケージがインストールされている場合）
    if docker-compose -f docker-compose.local.yml exec -T strategy-module python3 -c "import redis" 2>/dev/null; then
      docker-compose -f docker-compose.local.yml exec -T strategy-module python3 /app/scripts/ack_all_pending.py "$STREAM_NAME" "$GROUP_NAME"
    else
  ...
  ack-all-streams)
    ...
    for stream in md:ticker md:orderbook md:trade; do
      ...
      if docker-compose -f docker-compose.local.yml exec -T strategy-module python3 -c "import redis" 2>/dev/null; then
        docker-compose -f docker-compose.local.yml exec -T strategy-module python3 /app/scripts/ack_all_pending.py "$stream" "$GROUP_NAME"
      else
  ...
  *)
    ...
    echo "  GROUP_NAME   Consumer group name (default: strategy-module)"
    ...
    echo "  $0 md:trade strategy-module check         # Check pending messages for md:trade"
    echo "  $0 md:ticker strategy-module ack-all      # ACK all pending messages for md:ticker (recommended)"
    echo "  $0 '' '' ack-all-streams                  # ACK all pending messages for all streams (recommended)"
    echo "  $0 md:ticker strategy-module clear        # Clear by destroying consumer group"
